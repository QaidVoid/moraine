...
/// directory precedes its contents.
pub fn scan(image_dir: &Path) -> Result<Vec<ImageItem>, MergeError> {
    if !image_dir.exists() {
    ...
    let mut out = Vec::new();
    walk(image_dir, image_dir, &mut out)?;
...
fn walk(root: &Path, dir: &Path, out: &mut Vec<ImageItem>) -> Result<(), MergeError> {
